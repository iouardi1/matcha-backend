    ...
    createGoogleAccount: async ({
        ...
        if (!rows.length) {
            const googleUser = await db.query(
                'INSERT INTO users (firstname, lastname, email, auth_provider, provider_id, verified_account) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
                [given_name, family_name, email, provider, id, true]
            )
        }
    },
